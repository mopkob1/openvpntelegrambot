...
The [`ovpn_getclient`](/images/ovpn/bin/ovpn_getclient) can produce two different versions of the configuration.
...
If you have more than a few clients, you will want to generate and update your client configuration in batch. For this task the script [`ovpn_getclient_all`](/images/ovpn/bin/ovpn_getclient_all) was written, which writes out the configuration for each client to a separate directory called `clients/$cn`.
...
Revoke `client1`'s certificate and generate the certificate revocation list (CRL) using [`ovpn_revokeclient`](/images/ovpn/bin/ovpn_revokeclient) script :
